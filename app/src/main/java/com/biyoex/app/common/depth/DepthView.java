package com.biyoex.app.common.depth;
...
public class DepthView extends View {
    ...
    //手指单击松开后，数据是否继续显示
    private boolean isShowDetailSingleClick = false;
    //单击松开，数据延时消失，单位毫秒
    private final int DISAPPEAR_TIME = 500;
    //手指长按松开后，数据是否继续显示
    private boolean isShowDetailLongPress = false;
    //长按触发时长，单位毫秒
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        double avgWidthPerSize = (rightEnd - leftStart) / (buyDataList.size() + sellDataList.size()-2);
