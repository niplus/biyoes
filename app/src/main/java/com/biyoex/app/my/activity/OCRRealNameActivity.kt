...
import com.biyoex.app.my.presenter.OCRPresenter
import com.biyoex.app.my.view.OCRView
import id.zelory.compressor.Compressor
import io.reactivex.android.schedulers.AndroidSchedulers
import io.reactivex.schedulers.Schedulers
import kotlinx.android.synthetic.main.activity_ocrreal_name.*
...
class OCRRealNameActivity : BaseActivity<OCRPresenter>(),OCRView {

    var mSign:String = ""
    var mSignVersion = "hmac_sha1"
    var bizToken = ""
    private val CAMERA_PERMISSION_REQUEST_CODE = 100
    private val EXTERNAL_STORAGE_REQ_WRITE_EXTERNAL_STORAGE_CODE = 101
    ...
    override fun getLayoutId(): Int = R.layout.activity_ocrreal_name

    override fun createPresent(): OCRPresenter  = OCRPresenter()


    override fun initData() {
        tv_title.text = getString(R.string.certification)
        tv_right.text = "Authentication"
    ...
    override fun initComp() {
        ...
        if(edit_card_id.text.isNullOrEmpty()){ //验证是否输入姓名
            showToast(getString(R.string.input_id_card))
        }
            else if(edit_bank_id.text.isNullOrEmpty()){ //银行卡是否为null
            showToast(getString(R.string.input_bank_card))
        }
            else if(!RegexUtils.checkBankCard(edit_bank_id.text.toString())){ //银行卡格式是否正确
...
//        }
            ...
            showLoadingDialog()
            //请求4要素
            mPresent.checkBankAndName(edit_name.text.toString(),edit_card_id.text.toString(),edit_bank_id.text.toString())
        }
        }
    }
    //上传手持照片
    private fun postHoldPositve() {
        MPermissionUtils.requestPermissionsResult(this@OCRRealNameActivity, object : MPermissionUtils.OnPermissionListener {
            override fun onPermissionGranted() {
                if (strHoldPositiveUrl == "") {
                    MultiImageSelector.create(this@OCRRealNameActivity)
                            .showCamera(true)
                            .single()
                            .start(this@OCRRealNameActivity, RESULT_CODE_HOLD_POSITIVE)
                    //                            startActivityForResult(intent, RESULT_CODE_HOLD_POSITIVE);
                } else {
                    val itLookPicture = Intent(this@OCRRealNameActivity, LookPictureActivity::class.java)
            ...
            override fun onPermissionDenied() {
                MPermissionUtils.showTipsDialog(this@OCRRealNameActivity)
            }
            ...
            override fun onPermissionGranted() {
                if (strReverseSideUrl == "") {
                    MultiImageSelector.create(this@OCRRealNameActivity)
                            .showCamera(true)
                            .single()
                            .start(this@OCRRealNameActivity, RESULT_CODE_REVERSESIDE)
            ...
            override fun onPermissionGranted() {
                ...
                if (strPositiveUrl == "") {
                    MultiImageSelector.create(this@OCRRealNameActivity)
                            .showCamera(true)
                            .single()
                            .start(this@OCRRealNameActivity, RESULT_CODE_POSITIVE)
    ...
    private fun init() {
        FaceIdManager.getInstance(this@OCRRealNameActivity).init(bizToken)
        showLoadingDialog()

    }
    private val mInitListener = object : FaceIdInitListener {
        override fun onSuccess() {
            hideLoadingDialog()
            FaceIdManager.getInstance(this@OCRRealNameActivity).startDetect()
        }

        override fun onFailed(code: Int, msg: String) {}
    }

    private val mDetectListener = object : FaceIdDetectListener {
        override fun onSuccess(code: Int, msg: String) {
//            LogTools.getInstance().debug("验证结果---onSuccess:" + code + "----" + msg);
            Log.d("验证结果---onSuccess:" + code + "----" + msg)
            if (code == 51000) {
                showToast( "实名认证成功！")
                //提交资料

                mPresent.postRealName(edit_name.text.toString(),edit_card_id.text.toString(),edit_bank_id.text.toString(),0,
                        strPositiveUrl,strReverseSideUrl,strHoldPositiveUrl
                        )
            } else {
                showToast( "认证失败，请重新提交审核！")
            }
        }
    ...
    // 请求四要素返回结果
    override fun checkBankSuccess(biz_token:String) {
        bizToken = biz_token
        init()
