...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        float margin = ViewUtil.Dp2Px(mContext, 6);
        float width = 0;
        float left;
        float top = margin + view.getTopPadding();
        float height = padding * 8 + textHeight * 9;
        ICandle point = (ICandle) view.getItem(index);
        float lastprice = point.getClosePrice() - point.getlastClosePrice();
        List<String> strings = new ArrayList<>();
//        Log.i("时间噢", "drawSelector: " + index);
        strings.add(mContext.getString(R.string.kline_time) + view.getAdapter().getDate(index));
        strings.add(mContext.getString(R.string.buy_max_price) + new BigDecimal(point.getOpenPrice()).setScale(6,BigDecimal.ROUND_DOWN).toPlainString());
        strings.add(mContext.getString(R.string.max_price) +  new BigDecimal(point.getHighPrice()).setScale(6,BigDecimal.ROUND_DOWN).toPlainString());
        strings.add(mContext.getString(R.string.min_price) +  new BigDecimal(point.getLowPrice()).setScale(6,BigDecimal.ROUND_DOWN).toPlainString());
        strings.add(mContext.getString(R.string.sell_price) +  new BigDecimal(point.getClosePrice()).setScale(6,BigDecimal.ROUND_DOWN).toPlainString());
        strings.add(mContext.getString(R.string.up_down) + (point.getlastClosePrice() == 0f ? "0" : getValueFormatter().format(new BigDecimal(lastprice).floatValue())));
        strings.add(mContext.getString(R.string.up_and_down_number) + (point.getlastClosePrice() == 0f ? "0" : (new BigDecimal(lastprice).divide(new BigDecimal(point.getClosePrice()), 4, BigDecimal.ROUND_HALF_DOWN)).multiply(new BigDecimal(100)).setScale(2)) + "%");
        strings.add(mContext.getString(R.string.number) + point.getVolme());
